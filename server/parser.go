...
func (c *client) parse(buf []byte) error {
	fmt.Println("[parse]", "parser.start")
...
func protoSnippet(start, max int, buf []byte) string {
	fmt.Println("[protoSnippet]", "parser.start")
	stop := start + max
	bufSize := len(buf)
	if start >= bufSize {
		return `""`
...
func (c *client) overMaxControlLineLimit(arg []byte, mcl int32) error {
	fmt.Println("[overMaxControlLineLimit]", "parser.start")
	if c.kind != CLIENT {
		return nil
	}
	if len(arg) > int(mcl) {
...
func (c *client) clonePubArg(lmsg bool) error {
	fmt.Println("[clonePubArg]", "parser.start")
...
func (ps *parseState) getHeader() http.Header {
	fmt.Println("[getHeader]", "parser.start")
